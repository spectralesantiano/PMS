...
Public Class WORKDUE
    ...
    Private Sub EditData()
        If Not MainView.IsGroupRow(MainView.FocusedRowHandle) And (bPermission And 4) > 0 Then
            ...
            If frm.IS_SAVED Then
                ...
                Dim macname As String = MainView.GetFocusedRowCellValue("UnitDesc").Split(New String() {" ->"}, 0)(0)

                LastUpdatedBy = clsAudit.AssembleLastUBy(USER_REAL, "Edit", 10, System.Environment.MachineName,
                                           Replace(MainView.GetFocusedRowCellValue("UnitDesc"), "->", " - ").Substring(macname.Length) & " : " & frm.cboMaintenance.Text,
                ...
                sqls.Add("Insert Into dbo.tblMaintenanceWork([UnitCode],[MaintenanceCode],[ExecutedBy],[RankCode],[WorkDate],[WorkCounter],[Remarks],[DueCounter],[DueDate],[LastUpdatedBy],[bNC],[HasImage],[Locked],[DateAdded],[PrevDueDate],[PrevDueCounter],[CreatedBy]) Values('" &
                         frm.cboUnit.EditValue & "', '" & frm.cboMaintenance.EditValue & "', '" & frm.txtExecutedBy.EditValue.ToString.Replace("'", "''") & "', '" & frm.cboRankCode.EditValue & "'," & ChangeToSQLDate(frm.txtWorkDate.EditValue) & "," & strCounter & ",'" &
                         frm.txtRemarks.EditValue.ToString.Replace("'", "''") & "'," & strDueCounter & "," & strDateDue & ",'" & LastUpdatedBy & "',0," & IIf(frm.IView.RowCount > 0, 1, 0) & ", 0," & ChangeToSQLDate(Now.Date) & "," & IfNull(frm.pDueDate, "NULL") & "," & IfNull(frm.pDueCounter, "NULL") & "," & USER_ID & ")")
                ...
                For i = 0 To frm.MainView.RowCount - 1
                    If frm.MainView.GetRowCellValue(i, "Edited") Then
                        ...
                        LastUpdatedBy = clsAudit.AssembleLastUBy(USER_REAL, "Edit", 10, System.Environment.MachineName,
                                                                Replace(MainView.GetFocusedRowCellValue("UnitDesc"), "->", " - ").Substring(macname.Length) & " : " & frm.cboMaintenance.Text & " : Part " & frm.MainView.GetRowCellDisplayText(i, "Part"),
                                                                strCaption, , 1, MainView.GetFocusedRowCellValue("UnitDesc").Split(New String() {" ->"}, 0)(0), WORKTYPE, IIf(MainView.GetFocusedRowCellValue("Critical"), 1, 2), , frm.cboRankCode.Text) 'neil
                ...
                If frm.strDeletedImages <> "" Then
                    ...
                    LastUpdatedBy = clsAudit.AssembleLastUBy(USER_REAL, "", 10, System.Environment.MachineName,
                                                                Replace(MainView.GetFocusedRowCellValue("UnitDesc"), "->", " - ").Substring(macname.Length) & " : " & frm.cboMaintenance.Text & " : Document",
                                                                strCaption, , 1, MainView.GetFocusedRowCellValue("UnitDesc").Split(New String() {" ->"}, 0)(0), WORKTYPE, IIf(MainView.GetFocusedRowCellValue("Critical"), 1, 2), , frm.cboRankCode.Text) 'neil
                ...
                If frm.strAddedImages <> "" Then
                    ...
                    For Each strImg In strImages
                        ...
                        LastUpdatedBy = clsAudit.AssembleLastUBy(USER_REAL, "", 10, System.Environment.MachineName,
                                                          Replace(MainView.GetFocusedRowCellValue("UnitDesc"), "->", " - ").Substring(macname.Length) & " : " & frm.cboMaintenance.Text & " : Document " & strImg,
                                                          strCaption, , 1, MainView.GetFocusedRowCellValue("UnitDesc").Split(New String() {" ->"}, 0)(0), WORKTYPE, IIf(MainView.GetFocusedRowCellValue("Critical"), 1, 2), , frm.cboRankCode.Text) 'neil
    ...
    Public Overrides Sub AddData()
        ...
        If frm.IS_SAVED Then
            ...
            DB.RunSql("Insert Into dbo.tblMaintenanceWork([UnitCode],[MaintenanceCode],[ExecutedBy],[RankCode],[WorkDate],[WorkCounter],[Remarks],[DueCounter],[DueDate],[LastUpdatedBy],bNC,[CreatedBy]) Values('" &
                      strID & "', '" & frm.cboMaintenance.EditValue & "', '" & frm.txtExecutedBy.EditValue.ToString.Replace("'", "''") & "', '" & frm.cboRankCode.EditValue & "'," &
                      ChangeToSQLDate(frm.txtWorkDate.EditValue) & "," & strCounter & ",'" & frm.txtRemarks.EditValue.ToString.Replace("'", "''") & "'," & strDueCounter & "," & strDateDue & ",'" & LastUpdatedBy & "',0," & USER_ID & ")")
        End If
        RefreshData()
    End Sub
        ...
        If Not MainView.IsGroupRow(MainView.FocusedRowHandle) Then
            If MsgBox("Are you sure want to remove the <" & IfNull(MainView.GetFocusedRowCellDisplayText("Maintenance"), "New Record") & "> Work Record?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                If Not MainView.GetFocusedRowCellValue("MaintenanceWorkID") Is System.DBNull.Value Then
                    ...
                    Dim macname As String = MainView.GetFocusedRowCellValue("UnitDesc").Split(New String() {" ->"}, 0)(0)
                    LastUpdatedBy = clsAudit.AssembleLastUBy(USER_REAL, "", 10, System.Environment.MachineName,
                                            Replace(MainView.GetFocusedRowCellValue("UnitDesc"), "->", " - ").Substring(macname.Length) & " : " & MainView.GetFocusedRowCellDisplayText("Maintenance"),
                                            strCaption, , 1, strDesc.Split(New String() {" ->"}, 0)(0),
                                            ...
                                            IIf(MainView.GetFocusedRowCellValue("Critical"), 1, 2), ,
                                            MainView.GetFocusedRowCellDisplayText("Rank")) 'neil
